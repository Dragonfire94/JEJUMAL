...
| 중복 제거한 예문 | **524,406** |
| 방언 토큰 | **223,067** |
| 앱에 붙인 예문 | 말뭉치는 앱에 안 씀. 앱 예문은 `scripts/rewrite_examples.py`가 표준어 문장을 만든 뒤 `scripts/build_clean_examples.py`가 문장 전체를 제주어로 바꿈 (1,000개) |
...
앱 예문은 말뭉치에서 안 가져갑니다. 대화 조각이 학습용으로 안 맞아서, 표준어 문장을 만든 뒤 문장 전체를 제주어로 바꿉니다.

```bash
python scripts/rewrite_examples.py
```
