...
- P1-G부터 일부 예문은 제주특별자치도·제주학연구센터의 제주어 구술자료집(2017–2020)에서 채택한다.
- 해당 구술자료집 아카이브는 공공누리 제1유형(출처표시)으로 공개된 자료를 사용한다.
- 각 예문의 source.sourceId에 실제 구술 페이지 URL을 보존한다.
- 앱 설정 화면 정보 영역에서 출처와 공공누리 이용조건 링크를 제공한다.
- 같은 플랫폼의 별도 '제주어 교육 자료'에는 공공누리 제4유형 자료도 있으므로, 별도 이용허락 없이 그 PDF 문구를 상업용 앱 예문으로 복제·개작하지 않는다.
