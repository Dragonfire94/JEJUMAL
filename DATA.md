...
- 앱 예문: 표준어로 짧은 문장을 만든 뒤, 그 문장 전체를 제주어로 바꿉니다. 사전 단어는 표제어로, 해요체는 수다/우다/옵서체로. `scripts/rewrite_examples.py`가 단어마다 다른 서술부를 쓰고, `scripts/build_clean_examples.py`가 문장 전체를 제주어로 바꿉니다. 1,000단어 모두 예문 1개.
